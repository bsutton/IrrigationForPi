...
Note the output of certbot and inparticular the following two lines.
The paths are the important bit may be different from the below examples:
```
...
You MAY need to replace the the paths for SSLCertificateFile and SSLCertificateKeyFile if
the paths generated by certbot differ from the example paths in the below file.

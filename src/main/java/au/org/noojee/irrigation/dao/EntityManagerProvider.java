 ...
 * e.g.
 *
 * @formatter:off EntityManager em = EntityManagerProvider.createEntityManager(); 
 *                Transaction t = new Transaction(em); 
 *                try 
 *                { 
 *                	// Create andset the entity manager
...
public enum EntityManagerProvider
{
	...
	/**
	 * provides a mechanism to register actions that should happen should happen before a transaction is started and
	...
	/**
	 * Call this method to initialise the EntityManagerProvider so that it can hand out EntityManagers to worker
	 * threads. Dont forget to close the entitymanager This should normally be called from a servlet Context Listener.
	 *
	 * @param emf
	 */
	...
	/**
	 * provides the same functionality as setThreadLocalEntityManager(EntityWorker w), without the need for an anonymous
	 * inner class<br>
	 * example usage:<br>
	 * <br>
	 * try(AutoCloseable closer = EntityManagerProvider.setThreadLocalEntityManagerTryWithResources())<br>
	 * {<br>
	 * ...<br>
	 * }<br>
	 * 
	...
	/**
	 * Allows you to pass a Runnable to wrap in an entity manager. A new Runnable is returned which should then be
	 * called to run your runnable. i.e. don't run you own runnable directly rather use the returned Runnable.
	 *
	...
	/**
	 * Allows you to pass in a Callable to wrap in an entity manager. A new Callable is returned which should then be
	 * called to run your Callable. i.e. don't run you own Callable directly rather use the returned Callable.
	 *
	...
	/**
	 * If you have a worker thread then it won't have access to a thread local entity manager (as they are injected by
	 * the servlet request filters mentioned above. <br>
	 * <br>
	 * <b>For worker threads preferably use setThreadLocalEntityManager</b> <br>
	 * <br>
	 * Otherwise you need to call this method to get an entity manager. You will also need to call close when done
	 *
	 * @return
	 */
	...
	/**
	 * Adds a runnable to the list of Actions that will be performed after the entity manager for this thread has been
	 * cleared. NOTE: as the EM has been cleared the Runnable must NOT try any database operations as they will fail.
	 *
	...
	/**
	 * this is useful when you need to run code in a separate transaction and thread after the current transaction
	 * commits
	 */
	...
	/**
	 * Register an action that will be executed before begin is called on the transaction. Registered actions are global
	 * and will be run across all threads every time an entity manager is set via void
	...
	/**
	 * Evicts the entity of the specified class with the specified key in the JPA cache
	 */
